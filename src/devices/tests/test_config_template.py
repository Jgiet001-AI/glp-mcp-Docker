...
"""
Tests for config components in devices MCP server.
...
class TestSettings:
    ...
    def test_settings_service_specific_config(self):
        """Test service-specific configuration for devices."""
...
class TestLogging:
    ...
    def test_setup_logging_default(self):
        ...
        logger = get_logger("test_devices")
    ...
    def test_setup_logging_with_level(self):
        ...
        for level in levels:
            ...
            logger = get_logger("test_level_devices")
    ...
    def test_get_logger_names(self):
        """Test logger naming for devices."""
    ...
    def test_logging_output_format(self):
        ...
        setup_logging()
        logger = get_logger("format_test_devices")

        # Test that logger can handle different log levels
        logger.info("Test info message for devices")
        logger.warning("Test warning message for devices")
        logger.error("Test error message for devices")
    ...
    def test_structured_logging(self):
        ...
        setup_logging()
        logger = get_logger("structured_test_devices")
    ...
    def test_logging_with_environment_config(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            setup_logging()
            logger = get_logger("env_test_devices")

            # Should create logger successfully with env config
            assert logger is not None

    def test_logger_hierarchy(self):
        """Test logger hierarchy for devices components."""
        setup_logging()

        # Create loggers with hierarchical names
        root_logger = get_logger("devices")
        auth_logger = get_logger("devices.auth")
        server_logger = get_logger("devices.server")
...
class TestConfigIntegration:
    ...
    def test_settings_and_logging_integration(self):
        ...
        logger = get_logger("integration_test_devices")
    ...
    def test_config_validation_with_dependencies(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            # Validate required fields for devices
...
class TestConfigSecurity:
    ...
    def test_sensitive_data_not_logged(self):
        ...
        setup_logging()
        logger = get_logger("security_test_devices")

        # Create settings with sensitive data
        test_env = {
            "GREENLAKE_CLIENT_ID": "test-client-id-security",
            "GREENLAKE_CLIENT_SECRET": "super-secret-value-devices",
            "GREENLAKE_WORKSPACE_ID": "test-workspace-id-security",
            "TOKEN_ISSUER": "https://secure.devices.com/oauth2/token",
        }
...
def mock_config_file():
    ...
    config_content = """
    CLIENT_ID=mock-client-id-devices
    CLIENT_SECRET=mock-client-secret-devices
    TOKEN_ISSUER=https://mock.devices.com/oauth2/token
...
class TestConfigEdgeCases:
    ...
    def test_unicode_config_values(self):
        ...
        unicode_env = {
            "CLIENT_ID": "test-中文-devices",
            "TOKEN_ISSUER": "https://тест.devices.com/oauth2/token",
        }
