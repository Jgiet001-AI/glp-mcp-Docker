...
"""
Tests for dynamic tools in devices MCP server.
...
class TestListEndpointsTool:
    ...
    def test_tool_properties(self, tool):
        ...
        assert "devices" in tool.description
...
class TestGetEndpointSchemaTool:
    ...
    async def test_execute_valid_endpoint(self, tool):
        ...
        result = await tool.execute({"endpoint_identifier": "GET:/devices/v1/devices"})
    ...
    async def test_execute_with_examples(self, tool):
        ...
        result = await tool.execute({"endpoint_identifier": "GET:/devices/v1/devices", "include_examples": True})
...
class TestInvokeDynamicTool:
    ...
    async def test_execute_valid_get_request(self, tool, mock_http_client):
        ...
        with patch.object(tool, "http_client", mock_http_client):
            result = await tool.execute(
                {"endpoint_identifier": "GET:/devices/v1/devices", "parameters": test_parameters}
            )
    ...
    async def test_execute_unsupported_method(self, tool):
        ...
        result = await tool.execute(
            {
                "endpoint_identifier": "POST:/devices/v1/devices",  # Valid path but unsupported method
                "parameters": {},
            }
        )
...
def sample_tool_arguments():
    ...
    return {
        ...
        "get_endpoint_schema": {
            "endpoint_identifier": "GET:/devices/v1/devices",
            "include_examples": True,
            "include_validation": True,
        },
        "execute_dynamic_tool": {"endpoint_identifier": "GET:/devices/v1/devices", "parameters": {}, "headers": {}},
    }
