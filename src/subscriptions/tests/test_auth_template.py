...
"""
Tests for authentication components in subscriptions MCP server.
...
class TestTokenInfo:
    ...
    def test_token_info_creation(self):
        ...
        token_info = TokenInfo(
            token="test-token-subscriptions",
            expires_at=time.time() + 3600,
        )

        assert token_info.token == "test-token-subscriptions"
...
class TestOAuth2Provider:
    ...
    def oauth2_provider(self):
        ...
        return OAuth2Provider(
            ...
            token_url="https://subscriptions.example.com/oauth2/token",
    ...
    def mock_token_response(self):
        ...
        return {
            "access_token": "test-access-token-subscriptions",
            "token_type": "Bearer",
            "expires_in": 3600,
            "scope": "read write",
        }
    ...
    def test_get_token_success(self, mock_client_class, mock_token_response):
        """Test successful token acquisition for subscriptions."""
        ...
        assert token.access_token == "test-access-token-subscriptions"
        ...
        mock_client_instance.post.assert_called_once_with(
            "https://subscriptions.example.com/oauth2/token",
    ...
    def test_get_token_failure(self, mock_client_class):
        """Test failed token acquisition for subscriptions."""
    ...
    def test_validate_token(self, oauth2_provider):
        """Test token validation for subscriptions."""
        # Test valid token
...
class TestTokenManager:
    ...
    def valid_token(self):
        ...
        return OAuth2TokenResponse(
            access_token="valid-token-subscriptions",
            token_type="Bearer",
            expires_in=3600,
            scope="read write",
        )
    ...
    def expired_token(self):
        ...
        return OAuth2TokenResponse(
            access_token="expired-token-subscriptions",
            token_type="Bearer",
    ...
    def test_token_manager_initialization(self):
        """Test TokenManager initialization for subscriptions."""
        with patch("auth.token_manager.OAuth2Provider") as mock_provider_class:
            ...
            valid_response = OAuth2TokenResponse(
                access_token="test-init-token-subscriptions",
                token_type="Bearer",
                expires_in=3600,
                scope="read write",
            )
            ...
            mock_settings.token_issuer = "https://subscriptions.example.com/oauth2/token"
    ...
    def test_get_valid_token_when_none_cached(self, mock_provider_class, mock_settings, valid_token):
        """Test getting token when none is cached for subscriptions."""
        # Setup mock provider
        mock_provider = Mock()
    ...
    def test_get_valid_token_when_valid_cached(self, mock_provider_class, mock_settings, valid_token):
        """Test getting token when valid one is cached for subscriptions."""
    ...
    def test_get_valid_token_when_expired_cached(self, mock_provider_class, mock_settings, expired_token, valid_token):
        """Test getting token when expired one is cached for subscriptions."""
        # Setup mock provider
        mock_provider = Mock()
        mock_provider.get_token.return_value = valid_token
        mock_provider_class.return_value = mock_provider
    ...
    def test_token_refresh_on_auth_error(self, mock_provider_class, mock_settings, valid_token):
        ...
        refreshed_token = OAuth2TokenResponse(
            access_token="refreshed-token-subscriptions",
            token_type="Bearer",
            expires_in=3600,
            scope="read write",
        )
    ...
    def test_get_authorization_header(self, mock_provider_class, mock_settings, valid_token):
        """Test getting authorization header for subscriptions."""
        # Setup mock provider
        mock_provider = Mock()
        mock_provider.get_token.return_value = valid_token
        mock_provider_class.return_value = mock_provider
...
class TestAuthIntegration:
    ...
    def test_end_to_end_token_flow(self, mock_client_class, mock_settings):
        """Test complete token acquisition and management flow for subscriptions."""
        # Setup mock httpx.Client
        mock_client_instance = Mock()
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "access_token": "integration-test-token-subscriptions",
            "token_type": "Bearer",
            "expires_in": 3600,
        ...
        assert "Authorization" in auth_headers
        assert "integration-test-token-subscriptions" in auth_headers["Authorization"]
