...
"""
Tests for utility components in subscriptions MCP server.
...
from utils.http_client import SubscriptionsHttpClient, get_http_client


class TestSubscriptionsHttpClient:
    """Test cases for SubscriptionsHttpClient."""
    ...
    def http_client(self, mock_token_manager):
        ...
        with patch("utils.http_client.TokenManager", return_value=mock_token_manager):
            return SubscriptionsHttpClient()
    ...
    async def test_put_request_success(self, http_client, mock_token_manager):
        ...
        update_data = {"name": "Updated Resource for subscriptions", "status": "modified"}
    ...
    async def test_request_with_query_params(self, http_client, mock_token_manager):
        ...
        query_params = {"filter": "active", "limit": 10, "service": "subscriptions"}
...
class TestHttpClientFactory:
    ...
    def test_get_http_client(self):
        ...
        with patch("utils.http_client.SubscriptionsHttpClient") as mock_client_class:
            mock_client = Mock()
...
class TestHttpClientIntegration:
    ...
    def integration_token_manager(self):
        ...
        mock_manager = Mock()
        mock_manager.get_auth_headers.return_value = {
            "Authorization": "Bearer integration-token-subscriptions",
            "Content-Type": "application/json",
        }
        return mock_manager
    ...
    async def test_full_request_cycle(self, integration_token_manager):
        ...
        with patch("utils.http_client.TokenManager", return_value=integration_token_manager):
            ...
            with patch.object(http_client.client, "get") as mock_get:
                mock_response = Mock()
                mock_response.status_code = 200
                mock_response.json.return_value = {"service": "subscriptions", "status": "healthy", "version": "1.0.0"}
                mock_response.raise_for_status.return_value = None
                mock_get.return_value = mock_response
                ...
                # Verify result
                assert result["service"] == "subscriptions"
                assert result["status"] == "healthy"

                # Verify authentication was included
                call_args = mock_get.call_args
                headers = call_args.kwargs.get("headers", {})
                assert "integration-token-subscriptions" in headers.get("Authorization", "")
...
class TestHttpClientErrorHandling:
    ...
    async def test_large_response_handling(self, error_test_client):
        ...
        large_data = {
            "items": [{"id": i, "name": f"Item {i} for subscriptions", "data": "x" * 1000} for i in range(100)]
        }
        ...
        with patch.object(error_test_client.client, "get") as mock_get:
            ...
            assert result["items"][0]["name"] == "Item 0 for subscriptions"
...
def sample_api_response():
    """Sample API response data for subscriptions."""
    return {
        "id": "sample-id-subscriptions",
...
def sample_error_response():
    ...
    return {
        "error": {
            ...
            "message": "Resource not found in subscriptions",
            "details": {"service": "subscriptions", "timestamp": "2024-01-01T00:00:00Z"},
        }
    }
