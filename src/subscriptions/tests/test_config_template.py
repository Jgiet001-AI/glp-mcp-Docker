...
"""
Tests for config components in subscriptions MCP server.
...
class TestSettings:
    ...
    def test_settings_service_specific_config(self):
        """Test service-specific configuration for subscriptions."""
...
class TestLogging:
    ...
    def test_setup_logging_default(self):
        ...
        logger = get_logger("test_subscriptions")
    ...
    def test_setup_logging_with_level(self):
        ...
        for level in levels:
            ...
            logger = get_logger("test_level_subscriptions")
    ...
    def test_get_logger_names(self):
        """Test logger naming for subscriptions."""
        # Test various logger names
        test_names = [
            "subscriptions.auth",
            "subscriptions.server",
            "subscriptions.tools",
            "subscriptions.http_client",
        ]
    ...
    def test_logging_output_format(self):
        ...
        setup_logging()
        logger = get_logger("format_test_subscriptions")

        # Test that logger can handle different log levels
        logger.info("Test info message for subscriptions")
        logger.warning("Test warning message for subscriptions")
        logger.error("Test error message for subscriptions")
    ...
    def test_structured_logging(self):
        ...
        setup_logging()
        logger = get_logger("structured_test_subscriptions")
    ...
    def test_logging_with_environment_config(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            setup_logging()
            logger = get_logger("env_test_subscriptions")

            # Should create logger successfully with env config
            assert logger is not None

    def test_logger_hierarchy(self):
        """Test logger hierarchy for subscriptions components."""
        setup_logging()

        # Create loggers with hierarchical names
        root_logger = get_logger("subscriptions")
        auth_logger = get_logger("subscriptions.auth")
        server_logger = get_logger("subscriptions.server")
...
class TestConfigIntegration:
    ...
    def test_settings_and_logging_integration(self):
        ...
        logger = get_logger("integration_test_subscriptions")
...
class TestConfigSecurity:
    ...
    def test_sensitive_data_not_logged(self):
        ...
        setup_logging()
        logger = get_logger("security_test_subscriptions")

        # Create settings with sensitive data
        test_env = {
            "GREENLAKE_CLIENT_ID": "test-client-id-security",
            "GREENLAKE_CLIENT_SECRET": "super-secret-value-subscriptions",
            "GREENLAKE_WORKSPACE_ID": "test-workspace-id-security",
            "TOKEN_ISSUER": "https://secure.subscriptions.com/oauth2/token",
        }
...
def mock_config_file():
    ...
    config_content = """
    CLIENT_ID=mock-client-id-subscriptions
    CLIENT_SECRET=mock-client-secret-subscriptions
    TOKEN_ISSUER=https://mock.subscriptions.com/oauth2/token
...
class TestConfigEdgeCases:
    ...
    def test_unicode_config_values(self):
        ...
        unicode_env = {
            "CLIENT_ID": "test-中文-subscriptions",
            "TOKEN_ISSUER": "https://тест.subscriptions.com/oauth2/token",
        }
