...
"""
Tests for config components in workspaces MCP server.
...
class TestSettings:
    ...
    def test_settings_service_specific_config(self):
        """Test service-specific configuration for workspaces."""
...
class TestLogging:
    ...
    def test_setup_logging_default(self):
        ...
        logger = get_logger("test_workspaces")
    ...
    def test_setup_logging_with_level(self):
        ...
        for level in levels:
            ...
            logger = get_logger("test_level_workspaces")
    ...
    def test_get_logger_names(self):
        """Test logger naming for workspaces."""
        # Test various logger names
        test_names = [
            "workspaces.auth",
            "workspaces.server",
            "workspaces.tools",
            "workspaces.http_client",
        ]
    ...
    def test_logging_output_format(self):
        ...
        setup_logging()
        logger = get_logger("format_test_workspaces")

        # Test that logger can handle different log levels
        logger.info("Test info message for workspaces")
        logger.warning("Test warning message for workspaces")
        logger.error("Test error message for workspaces")
    ...
    def test_structured_logging(self):
        ...
        setup_logging()
        logger = get_logger("structured_test_workspaces")
    ...
    def test_logging_with_environment_config(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            setup_logging()
            logger = get_logger("env_test_workspaces")

            # Should create logger successfully with env config
            assert logger is not None

    def test_logger_hierarchy(self):
        """Test logger hierarchy for workspaces components."""
        setup_logging()

        # Create loggers with hierarchical names
        root_logger = get_logger("workspaces")
        auth_logger = get_logger("workspaces.auth")
        server_logger = get_logger("workspaces.server")
...
class TestConfigIntegration:
    ...
    def test_settings_and_logging_integration(self):
        ...
        logger = get_logger("integration_test_workspaces")
    ...
    def test_config_validation_with_dependencies(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            # Validate required fields for workspaces
...
class TestConfigSecurity:
    ...
    def test_sensitive_data_not_logged(self):
        ...
        setup_logging()
        logger = get_logger("security_test_workspaces")

        # Create settings with sensitive data
        test_env = {
            "GREENLAKE_CLIENT_ID": "test-client-id-security",
            "GREENLAKE_CLIENT_SECRET": "super-secret-value-workspaces",
            "GREENLAKE_WORKSPACE_ID": "test-workspace-id-security",
            "TOKEN_ISSUER": "https://secure.workspaces.com/oauth2/token",
        }
...
def mock_config_file():
    ...
    config_content = """
    CLIENT_ID=mock-client-id-workspaces
    CLIENT_SECRET=mock-client-secret-workspaces
    TOKEN_ISSUER=https://mock.workspaces.com/oauth2/token
...
class TestConfigEdgeCases:
    ...
    def test_unicode_config_values(self):
        ...
        unicode_env = {
            "CLIENT_ID": "test-中文-workspaces",
            "TOKEN_ISSUER": "https://тест.workspaces.com/oauth2/token",
        }
