...
"""
Tests for utility components in workspaces MCP server.
...
class TestWorkspacesHttpClient:
    """Test cases for WorkspacesHttpClient."""
    ...
    async def test_put_request_success(self, http_client, mock_token_manager):
        ...
        update_data = {"name": "Updated Resource for workspaces", "status": "modified"}
    ...
    async def test_request_with_query_params(self, http_client, mock_token_manager):
        ...
        query_params = {"filter": "active", "limit": 10, "service": "workspaces"}
...
class TestHttpClientIntegration:
    ...
    def integration_token_manager(self):
        ...
        mock_manager = Mock()
        mock_manager.get_auth_headers.return_value = {
            "Authorization": "Bearer integration-token-workspaces",
            "Content-Type": "application/json",
        }
        return mock_manager
    ...
    async def test_full_request_cycle(self, integration_token_manager):
        ...
        with patch("utils.http_client.TokenManager", return_value=integration_token_manager):
            ...
            with patch.object(http_client.client, "get") as mock_get:
                mock_response = Mock()
                mock_response.status_code = 200
                mock_response.json.return_value = {"service": "workspaces", "status": "healthy", "version": "1.0.0"}
                mock_response.raise_for_status.return_value = None
                mock_get.return_value = mock_response
                ...
                assert "integration-token-workspaces" in headers.get("Authorization", "")
...
class TestHttpClientErrorHandling:
    ...
    async def test_large_response_handling(self, error_test_client):
        ...
        large_data = {"items": [{"id": i, "name": f"Item {i} for workspaces", "data": "x" * 1000} for i in range(100)]}
        ...
        with patch.object(error_test_client.client, "get") as mock_get:
            ...
            assert result["items"][0]["name"] == "Item 0 for workspaces"
...
def sample_api_response():
    """Sample API response data for workspaces."""
    return {
        "id": "sample-id-workspaces",
...
def sample_error_response():
    ...
    return {
        "error": {
            ...
            "message": "Resource not found in workspaces",
            "details": {"service": "workspaces", "timestamp": "2024-01-01T00:00:00Z"},
        }
    }
