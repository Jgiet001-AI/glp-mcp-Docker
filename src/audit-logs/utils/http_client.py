...
"""
HTTP client for audit-logs MCP server.
...
class AuditLogsHttpClient:
    """HTTP client for audit-logs API with authentication."""
...
def get_http_client() -> "AuditLogsHttpClient":
    """Get the global HTTP client instance (lazy initialization)."""
    global _http_client
    if _http_client is None:
        _http_client = AuditLogsHttpClient()
    return _http_client
