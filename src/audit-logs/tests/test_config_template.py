...
"""
Tests for config components in audit-logs MCP server.
...
class TestSettings:
    ...
    def test_settings_service_specific_config(self):
        """Test service-specific configuration for audit-logs."""
...
class TestLogging:
    ...
    def test_setup_logging_default(self):
        ...
        logger = get_logger("test_audit_logs")
    ...
    def test_setup_logging_with_level(self):
        ...
        for level in levels:
            ...
            logger = get_logger("test_level_audit_logs")
    ...
    def test_get_logger_names(self):
        """Test logger naming for audit-logs."""
        # Test various logger names
        test_names = [
            "audit_logs.auth",
            "audit_logs.server",
            "audit_logs.tools",
            "audit_logs.http_client",
        ]
    ...
    def test_logging_output_format(self):
        ...
        setup_logging()
        logger = get_logger("format_test_audit_logs")

        # Test that logger can handle different log levels
        logger.info("Test info message for audit-logs")
        logger.warning("Test warning message for audit-logs")
        logger.error("Test error message for audit-logs")
    ...
    def test_structured_logging(self):
        ...
        setup_logging()
        logger = get_logger("structured_test_audit_logs")
    ...
    def test_logging_with_environment_config(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            setup_logging()
            logger = get_logger("env_test_audit_logs")

            # Should create logger successfully with env config
            assert logger is not None

    def test_logger_hierarchy(self):
        """Test logger hierarchy for audit-logs components."""
        setup_logging()

        # Create loggers with hierarchical names
        root_logger = get_logger("audit_logs")
        auth_logger = get_logger("audit_logs.auth")
        server_logger = get_logger("audit_logs.server")
...
class TestConfigIntegration:
    ...
    def test_settings_and_logging_integration(self):
        ...
        logger = get_logger("integration_test_audit_logs")
...
class TestConfigSecurity:
    ...
    def test_sensitive_data_not_logged(self):
        ...
        setup_logging()
        logger = get_logger("security_test_audit_logs")

        # Create settings with sensitive data
        test_env = {
            "GREENLAKE_CLIENT_ID": "test-client-id-security",
            "GREENLAKE_CLIENT_SECRET": "super-secret-value-audit-logs",
            "GREENLAKE_WORKSPACE_ID": "test-workspace-id-security",
            "TOKEN_ISSUER": "https://secure.audit-logs.com/oauth2/token",
        }
...
def mock_config_file():
    ...
    config_content = """
    CLIENT_ID=mock-client-id-audit-logs
    CLIENT_SECRET=mock-client-secret-audit-logs
    TOKEN_ISSUER=https://mock.audit-logs.com/oauth2/token
...
class TestConfigEdgeCases:
    ...
    def test_unicode_config_values(self):
        ...
        unicode_env = {
            "CLIENT_ID": "test-中文-audit-logs",
            "TOKEN_ISSUER": "https://тест.audit-logs.com/oauth2/token",
        }
