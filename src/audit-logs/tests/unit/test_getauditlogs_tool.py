...
"""
Test for getauditlogs tool in audit-logs MCP server.

This file contains tests for the getauditlogs tool.
"""

import pytest

# Import getauditlogsTool
from tools.implementations.getauditlogs import getauditlogsTool


class TestgetauditlogsTool:
    """Test cases for getauditlogsTool."""

    @pytest.fixture
    def tool_instance(self, mock_http_client):
        """Create tool instance for testing."""
        return getauditlogsTool(mock_http_client)
    ...
    def test_tool_name(self, tool_instance):
        ...
        assert tool_instance.name == "getauditlogs"
    ...
    async def test_execute_api_error(self, tool_instance, valid_arguments, mock_http_client):
        ...
        assert len(result) == 1
        assert result[0]["success"] is False
        assert result[0]["tool"] == "getauditlogs"
